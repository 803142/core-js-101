...
function getPolynom(a, b, c) {
  let result = null;
  if (!!a && !!b && !!c) { result = function res(x) { return a * x ** 2 + b * x + c; }; }
  if (!!a && !!b && !c) { result = function res(x) { return a * x + b; }; }
  if (!!a && !b && !c) { result = function res() { return a; }; }
  return result;
}
