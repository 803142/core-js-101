...
function findFirstSingleChar(str) {
  let result = null;
  let i = 0;
  while (!result && str.length > i) {
    if (!str.slice(i + 1).match(str[i]) && !str.slice(0, i).match(str[i])) result = str[i];
    i += 1;
  }
  return result;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  let path = '';
  let i = 0;
  const condition = (item) => item[i] === path1[i];
  while (pathes.every(condition)) {
    path += path1[i];
    i += 1;
  }
